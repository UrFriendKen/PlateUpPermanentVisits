...
namespace KitchenPermanentVisits.Patches
{
    ...
    static class TwitchNameList_Patch
    {
        ...
        const string DESCRIPTION = "Add conditional reset"; // Logging purpose of patch
        ...
        static readonly List<object> OPERANDS_TO_MATCH = new List<object>()
        {
            ...
            null,
            null,
            typeof(TwitchNameList).GetMethod("ClearData", BindingFlags.Public | BindingFlags.Instance)
        };
        ...
        static readonly List<object> MODIFIED_OPERANDS = new List<object>()
        {
            ...
            null,
            null,
            typeof(TwitchNameList_Patch).GetMethod("ConditionalClearData", BindingFlags.NonPublic | BindingFlags.Static)
        };

        private static void ConditionalClearData()
        {
            if (!Main.PrefManager.Get<bool>(Main.RESET_VISITS_ID))
            {
                Main.LogWarning("Skipped wiping visits. Reshuffling names.");
                if (!PatchHelper.StaticSet<TwitchNameList.CReshuffleNameList>(out Entity _))
                {
                    Main.LogError("Failed to shuffle name list");
                }
                return;
            }
            Main.LogInfo("Requesting visit wiping");
            if (!PatchHelper.StaticSet<TwitchNameList.CResetNameList>(out Entity _))
            {
                Main.LogError("Failed to reset name list");
            }
